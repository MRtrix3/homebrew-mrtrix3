...
class Qt5Requirement < Requirement
  ...
  satisfy build_env: false do
    @qmake = if File.file?("#{HOMEBREW_PREFIX}/opt/qt5/bin/qmake")
      which("#{HOMEBREW_PREFIX}/opt/qt5/bin/qmake")
    else
      which("qmake")
    end
    @qmake
...
class Mrtrix3 < Formula
  desc ": tools to perform various types of diffusion MRI analyses"
  ...
  option "without-multithreaded_build",
         "This is useful if your computer has many cores but not enough RAM to build MRtrix using multiple threads. Alternatively, set the environment variable HOMEBREW_NUMBER_OF_PROCESSORS=1"
  ...
  conflicts_with "mrtrix3_0rc3",
                 because: "tagged version (@0rc3) conflicts with non-tagged version of mrtrix3 (this)"
  ...
  def install
    ...
    if build.with? "stable"
      ...
      system "git", "checkout", latesttag.to_s
    end
    ...
    if build.without? "matlab"
      ...
      begin
        ...
        print "Unable to set Matlab path: #{e}\n"
      end
    end

    unless ENV["HOMEBREW_NUMBER_OF_PROCESSORS"].nil? #  ENV option: HOMEBREW_NUMBER_OF_PROCESSORS
      ENV["NUMBER_OF_PROCESSORS"] = ENV["HOMEBREW_NUMBER_OF_PROCESSORS"]
    end
    ENV["NUMBER_OF_PROCESSORS"] = "1" if build.without? "multithreaded_build"

    conf = ["./configure"]
    conf.push("-nogui") if build.without? "qt5"
    conf.push("-assert") if build.with? "assert"
    conf.push("-debug") if build.with? "debug"
    ...
    if File.directory?("release") # pre tag 0.3.16
      ...
      bld = ["./build"]
      bld.push("release/bin/mrconvert") if build.with? "mrconvert"
      bld.push("release/bin/mrinfo") if build.with? "mrinfo"
      bld.push("release/bin/mrview") if build.with? "mrview"
      execute(bld.join(" "))

      # This has to be before bin.install or else binaries are not in place.
      if build.with? "test"
        tst = ["./run_tests"]
        tst.push("mrconvert") if build.with? "mrconvert"
        execute(tst.join(" "))
      ...
      bld = ["./build"]
      bld.push("bin/mrconvert") if build.with? "mrconvert"
      bld.push("bin/mrinfo") if build.with? "mrinfo"
      bld.push("bin/mrview") if build.with? "mrview"
      execute(bld.join(" "))
